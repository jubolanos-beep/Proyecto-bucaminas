...
void limpiar_casillas_8(char matriz[8][8], int matriz_int [8][8]){
	//Los ciclos iniciales recorren la matriz un n√É¬∫mero de veces igual al del n√É¬∫mero de columnas, con el de fin no dejar ning√É¬∫n cero que no este rodeado por n√É¬∫meros.
	for (int c=0; c<8; c++){
		if (matriz[0][0]=='0'){
...
void jugarNivelUno(Jugador ranking[], int n, string nombreJugador, string nombreArchivo){
	//Con thread se inicializan mas "hilos" en la ejecucion, que permiten que trozos de codigo se ejecuten a la vez que la funcion main, sin esto, el cronometro se quedara en un while infinito ya que nunca se llegara a la condicion de parada.
		...
		while (cerrar_ciclo_8x8==false){
			...
			cin>>jugada;
			//En estos case se separa la jugada como un vector de chars para pasar m√É¬°s f√É¬°cil las coordenadas a la matriz.
...
int main (){
	...
	do{
		system("cls");
		cout<<"=======MEN⁄ BUSCAMINAS============"<<endl<<"Escoja el nivel que desea jugar"<<endl<<"Oprima 1 para f·cil (8x8)"<<endl<<"Oprima 2 para medio (16x16)"<<endl<<"Oprima 3 para difÌcil (16x30)"<<endl<<"Oprima 4 para ver los mejores puntajes"<<endl<<"Oprima 0 para salir"<<endl;
	...
	/*while(nivel<1 || nivel>4){
		cout<<"Solo puede escoger un nivel escribiendo un n˙mero del 1 al 4"<<endl<<"Escoja un nivel: "<<endl;
		cin>>nivel;
	}*/
